...
int main(){
    ...
    vector<pair<long long , long long>>fresh_fruits;
    if(file.is_open()){
    ...
    long long ans = fresh_fruits[0].second - fresh_fruits[0].first+1;
    // fresh_fruits_map.
    cout<<ans<<endl;
    for(int i=1; i<fresh_fruits.size(); i++){
        if(fresh_fruits[i].first <= fresh_fruits[i-1].second){
            if(fresh_fruits[i].second > fresh_fruits[i-1].second){
                ans+=fresh_fruits[i].second-fresh_fruits[i-1].second;
            }
            fresh_fruits[i].second = max( fresh_fruits[i].second, fresh_fruits[i-1].second);
            

            // ans+=fresh_fruits[i].second-fresh_fruits[i-1].second;
        }
        else{
            ans+=fresh_fruits[i].second-fresh_fruits[i].first+1;
        }
        cout<<fresh_fruits[i].first<<" "<<fresh_fruits[i].second<<" "<< ans<<endl;
    }
    // unordered_set<long long>fresh;
